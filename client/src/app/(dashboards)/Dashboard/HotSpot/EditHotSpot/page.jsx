...
const EditHotSpot = () => {
  ...
  const handleSubmit = async (formData) => {
    ...
    let submitData = { ...data, hotspotSectionName: "sectionOne" };
  ...
  return (
    <div>
      <div className="flex justify-end">
       <ButtonWithModal title={`Back`} back path={'/Dashboard/HotSpot'}/>
      </div>
      <ZFormTwo
        isLoading={isLoading}
        isSuccess={isSuccess}
      ...
      >
        <div className="grid grid-cols-1 gap-3 mt-10">
          ...
          {hotData?.hotspotType === "link" &&
            hotData?.moduleName === "others" &&  (
            <ZInputTwo
              name="url"
              type="text"
              label="URL"
              placeholder="Enter the URL for the hotspot"
              value={hotData?.url}
              onChange={(e) => setUrlValue(e.target.value)}
            />
          )}

        {hotspotType === "link" && urlOption === "Others" && (
            <ZInputTwo
              name="url"
              type="text"
              label={"Url"}
