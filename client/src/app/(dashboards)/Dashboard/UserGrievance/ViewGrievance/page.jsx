  ...
  const ViewGrievance = () => {
    ...
    const handlePrint = () => {
      ...
      const imagesHtml = parsedProofs && parsedProofs.length > 0
      ? parsedProofs
          .map((proof ,index) => {
            const fileExtension = proof.file.split('.').pop().toLowerCase();
            const isImage = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'svg'].includes(fileExtension);
    
            const detailsHtml = `<p>Details: ${proof?.details ? proof.details : "No details given"}</p>`;
              ...
              return `
                ...
                <p><strong>${index + 1}. </strong> </p>
                  <img 
                    ...
                    alt="Proof" 
                    style="width: 200px; height: auto; margin: 10px; display: inline-block;" 
                  />
                </div>
                  ${detailsHtml}
                </div>
    ...
    ${styledGrievanceDetails}
          ...
          <p style="font-size: 22px; margin-bottom:300px;">${grievance?.user?.userPhone}</p>
          <p style="padding-top:90px;"> <strong> ${currentLanguage === 'en' ? "Proofs:" : "সংযুক্তি সমূহ:"}</strong></p>
          <p>${imagesHtml}</p>
        </div>
      </div>
    `;
